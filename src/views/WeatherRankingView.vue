...
const cleanestAir = computed(() => weather.value
  ...
  .slice(0, 5))
const rainiestObserved = computed(() => weather.value
  .filter((city) => city.precipitation > 0)
  .sort((a, b) => b.precipitation - a.precipitation)
  .slice(0, 5))
const rainiestForecast = computed(() => weather.value
...
const mapRegions = computed(() => weather.value
  .map((city) => {
  ...
  <main class="ranking-page">
    ...
    <div class="page-inner">
      ...
      <div v-else class="board-grid">
        <section v-for="board in boards" :key="board.id" class="board panel" :aria-labelledby="`board-${board.id}`">
          ...
          <ol v-if="board.items.length" class="board-list">
            <li v-for="(city, index) in board.items" :key="city.id" class="board-row">
              <span class="board-rank" aria-hidden="true">{{ index < 3 ? ['🥇', '🥈', '🥉'][index] : index + 1 }}</span>
...
<style scoped>
.ranking-page { width: 100%; padding-bottom: 72px; }
.page-inner { width: min(1120px, calc(100% - 40px)); margin: 0 auto; padding-top: 32px; }

.hero { position: relative; display: flex; align-items: flex-end; min-height: 360px; overflow: hidden; }
.hero-media { position: absolute; inset: 0; background-image: v-bind(heroImageStyle); background-position: center; background-size: cover; }
.hero-scrim { position: absolute; inset: 0; background: linear-gradient(180deg, rgba(15, 13, 9, .05) 0%, rgba(15, 13, 9, .35) 65%, rgba(15, 13, 9, .55) 100%); }
.hero-inner { position: relative; z-index: 1; width: 100%; padding: 0 max(20px, calc((100% - 1120px) / 2)) 44px; }
.eyebrow { margin: 0 0 9px; color: rgba(255, 255, 255, .85); font-size: .72rem; font-weight: 700; letter-spacing: .16em; }
h1 { margin: 0 0 12px; color: #fff; font-size: clamp(2rem, 5vw, 3.4rem); line-height: 1.1; letter-spacing: -.045em; font-weight: 700; }
.hero-copy { margin: 0; color: rgba(255, 255, 255, .85); font-size: 1.05rem; }

.highlight-banner { display: inline-flex; align-items: center; gap: 8px; margin: 0 0 24px; padding: 14px 20px; border: 1px solid var(--line); border-radius: 999px; color: var(--ink); background: var(--blue-100); font-size: .92rem; font-weight: 600; }

.state-message { display: grid; min-height: 200px; margin: 0; place-items: center; color: var(--muted); }
.error-result { display: grid; min-height: 200px; place-items: center; gap: 10px; text-align: center; }
.error-result p { margin: 0; color: var(--muted); }
.error-result button { padding: 9px 16px; border: 0; border-radius: 999px; color: #fff; background: var(--blue-700); font-weight: 600; }

.board-grid { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 20px; margin-bottom: 24px; }
.board { padding: 26px; border: 1px solid var(--line); border-radius: 20px; background: var(--surface); box-shadow: var(--shadow); }
.board-heading { display: flex; align-items: center; gap: 10px; margin-bottom: 16px; }
.board-icon { font-size: 1.4rem; }
.board-heading h2 { margin: 0; color: var(--ink); font-size: 1.1rem; font-weight: 700; }
.board-list { display: grid; gap: 0; margin: 0; padding: 0; list-style: none; }
.board-row { display: grid; grid-template-columns: 30px 1fr auto; align-items: center; gap: 10px; padding: 10px 2px; border-top: 1px solid var(--line); }
.board-row:first-child { border-top: 0; }
.board-rank { font-size: 1rem; text-align: center; }
...
.infrared-heading { margin-bottom: 18px; }
